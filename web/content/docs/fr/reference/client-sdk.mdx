...
## Entrées des matchers

Les matchers d’assertion sont publiés séparément du serveur, du client et des fixtures :

| Import | Exports principaux | Comportement |
| --- | --- | --- |
| `inboxtap/matchers` | `inboxTapMatchers`, `createInboxTapMatchers()` | Implémentations sans peer et types publics d’observation |
...
Vitest, ni Playwright. Seul un adaptateur propre à un runner a besoin du peer correspondant.
...
plus tardive est impossible. Le matcher one-click vérifie uniquement la forme des en-têtes RFC
8058. Il ne vérifie ni les signatures DKIM, ni leur couverture, ni le comportement de l’endpoint
de désinscription.

Les résultats des matchers omettent volontairement `actual`, `expected`, les corps de message, les
...
Pour collecter un rapport, transmettez un recorder synchrone à la fabrique pure :
...
Chaque observation est versionnée et enregistre le nom du matcher, la négation et l’état de
...
valeur d’un en-tête ou le corps capturé. Passez `matchers` à l’`expect.extend()` d’un runner
compatible ; les adaptateurs propres aux runners appellent la même fabrique.

## Rapports de test

Importez `InboxTapReport` depuis le sous-chemin sans peer `inboxtap/reports`. Le collecteur
implémente `InboxTapMatcherRecorder` : transmettez-le directement à un adaptateur de runner ou à
...
| `addAssertion({ name, passed, message?, messageId?, details? })` | Ajoute une assertion de l’application et renvoie le collecteur. Les chaînes facultatives et les `details` structurés passent par le même pipeline d’expurgation borné. |
| `recordMatcherObservation(observation)` | Implémente `InboxTapMatcherRecorder` ; les adaptateurs de runner l’appellent pour chaque matcher terminé. |
...
La portée du recorder suit l’objet `expect` étendu. L’exemple utilise l’`expect` de Vitest lié au
test, de sorte que chaque rapport ne reçoit que les observations des matchers de ce test. Ne
...
Le pipeline d’expurgation par défaut :
...
projet. La collecte est limitée à 100 messages et 1 000 assertions, et chaque artefact rendu est
...
L’expurgation est une protection au mieux et ne peut garantir la détection de données personnelles
ou secrètes arbitraires. Vérifiez l’artefact avant de le partager. `includeRaw: true` conserve une
copie expurgée au mieux de la source RFC brute et ne doit être utilisé que lorsque la valeur
